...
for $item in distinct-values($list) 
   for $bibl in collection($coll)//t:bibl[@corresp = $item]
	...
	let $opt := if($att = $repository) then element option { attribute value {$item,";",$att},  attribute selected {"selected"}, $tit } 
	else element option { attribute value {concat($item,";",$att)}, $tit } 
	return $opt
} </select>
