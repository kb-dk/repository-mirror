...
public class ApiClient {
	...
	private void init() {
		try {
			String xslResourceID = consts.getConstants().getProperty("xsl.add_id");
			InputStream in = this.getClass().getResourceAsStream(xslResourceID);
			StreamSource source = new StreamSource(in);

			this.transformer = trans_fact.newTransformer(source);
		} catch(TransformerConfigurationException xerror) {
			logger.info(logStackTrace(xerror), xerror);
		}
	}
	...
	public String restGet(String URI) {
		...
		try { if(httpClient != null) httpClient.close(); } catch(IOException e) {logger.info(logStackTrace(e));}
		return contents;
	}
	...
	public String restPut(String file, String URI) {
		...
		try {
			...
			if(apacheHttpApiWorks) {
				String text = transformText(file);
	...
	protected String transformText(String filePath) throws IOException {
		if(this.transformer != null) {
			...
			org.w3c.dom.Document doc = this.readDom(filePath);
	...
	public org.w3c.dom.Document readDom(String fileName) throws IOException {
	...
	public String doingItApacheWay(String text, String URI)  throws IOException {
